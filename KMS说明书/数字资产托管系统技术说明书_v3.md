...
| **DMZ21**  | Deng-Ma-Zhang 2021                      | SECP256K1 | ECDSA 签名（优化性能版本）     |
...
| FIPS 140-2 Level 3          | 密码模块安全要求     | HSM 兼容标准     |
...
系统采用分层架构设计，将业务逻辑与密钥管理完全解耦。所有组件部署在统一的虚拟私有云（VPC）环境中，通过网络策略实现逻辑隔离。
...
graph TD
    ...
    subgraph VPC["统一 VPC 环境"]
        C --> D["KMS 核心服务"]
        E["管理控制台"]
        
        D -->|"gRPC mTLS"| F["TSS 节点集群"]
...
| **KMS 网关**          | 对外 API 入口，负责身份认证、请求路由、流量控制                                | 统一 VPC   |
| **KMS 核心服务**      | 负责鉴权、风控策略检查及 MPC 任务调度                                          | 统一 VPC   |
| **管理控制台**        | 提供密钥管理、配置管理、审计查询的 Web 界面                                    | 统一 VPC   |
| **TSS 节点集群**      | 分布式部署的 MPC 计算节点，各节点持有密钥分片，协同完成签名计算                | 统一 VPC   |
| **MySQL**             | 存储密钥元数据（KeyID、地址、分片分布信息），**不存储分片内容**                | 统一 VPC   |
| **Redis**             | 缓存会话状态、限流计数等临时数据                                               | 统一 VPC   |
...
| 网络层     | IP 网段         | 访问策略                   |
| ---------- | --------------- | -------------------------- |
| 公网接入层 | 任意 (Internet) | 允许公网通过 IP 白名单访问 |
| 应用服务层 | 10.2.8.0/24     | 包含网关、控制台服务       |
| MPC 计算层 | 10.2.9.0/24     | 包含 TSS 节点集群          |
| 数据存储层 | 10.2.10.0/24    | 包含 MySQL、Redis          |
...
系统针对内部网络环境下的通信，实施了严格的 **IP 白名单** 访问控制策略作为关键安全屏障。

| 协议        | 访问控制      | 说明                                                                   |
| ----------- | ------------- | ---------------------------------------------------------------------- |
| HTTP (内部) | IP 白名单控制 | 仅允许预先登记的、受信任的业务系统 IP 地址访问 KMS 网关接口            |
|             |               | 所有来自非白名单 IP 的请求都将在网络接入层被直接拒绝，建立可信通信边界 |
...
**证书轮换：** 证书有效期为一年，遵循《密钥管理规范》定义的生命周期管理流程，由安全运维团队定期执行轮换操作，确保持续的通信安全。
...
系统采用分层密钥加密体系，根密钥由信息安全专员安全保管。系统架构设计兼容硬件安全模块（HSM）集成，支持在需要更高安全等级场景下的平滑升级。
...
| -------- | ------------------------------------------ |
| 根密钥   | 由信息安全专员在安全环境中保管             |
| DEK      | 使用密码加密后存储在 `.dek` 文件           |
| 密钥分片 | 使用 DEK 加密后存储在 TSS 节点本地 LevelDB |

**HSM 兼容能力：**

系统架构设计支持与符合 FIPS 140-2 Level 3 认证的硬件安全模块集成，可在需要时将根密钥管理迁移至硬件设备。
...
系统在技术层面实施了严格的身份验证与权限控制。

**访问控制措施：**

1. **链路控制：** 所有操作必须经过堡垒机跳转，拒绝任何直连尝试
2. **身份凭证：** 使用高强度 SSH 密钥（ED25519/RSA-4096）进行身份识别
3. **权限分离：** 不同角色的密钥分发严格隔离，开发人员无法访问生产节点
4. **会话审计：** 堡垒机自动记录所有操作历史，支持事后审计
...
系统采用标准化的 CI/CD 流水线进行代码构建与发布。

**构建流程：**

1. **版本控制：** 所有代码均托管于 Git 版本控制系统，关键分支受保护
2. **自动化构建：** 使用 Jenkins 流水线自动编译代码
3. **版本管理：** 代码发布带版本信息，便于追溯和管理

### 5.5 密钥生成与初始化流程

系统采用标准化的密钥生成流程，由授权的信息安全专员在安全环境中执行。

**流程步骤：**

1. **CA根密钥生成**
   - 在专用服务器上生成CA根密钥（RSA 4096位）
   - CA证书有效期设置为10年
   - 根密钥用于签发所有节点证书

2. **节点密钥生成**
   - 为每个TSS节点生成独立的DEK
   - DEK使用PBKDF2密钥派生函数加密（迭代次数≥10000）
   - 加密后的DEK存储在`{node_id}.dek`文件中

3. **密钥分发与部署**
   - 通过SSH加密通道将密钥文件分发至目标节点
   - 设置文件权限为600，仅服务进程可访问
   - 密钥文件与数据文件物理分离存储
...
| 控制点   | 实现方式                                              |
| -------- | ----------------------------------------------------- |
| 人员授权 | 仅授权的信息安全专员可执行密钥生成操作                |
| 环境隔离 | 密钥生成在独立的安全环境中进行                        |
| 密钥强度 | 使用符合NIST标准的高强度密钥算法                      |
| 加密存储 | DEK使用密码加密，采用PBKDF2密钥派生（迭代次数≥10000） |
| 权限控制 | 密钥文件设置严格的访问权限，仅服务进程可读取          |
| 启动验证 | 服务启动时需要人工输入密码，防止未授权启动            |
| 密码管理 | DEK加密密码由信息安全专员安全保管                     |
...
系统具备完善的灾难恢复能力，支持多种故障场景的快速恢复。

| 灾难级别 | 场景描述          | 应对策略                               | RPO           |
| -------- | ----------------- | -------------------------------------- | ------------- |
| L1       | 单个 TSS 节点故障 | 自动隔离故障节点，依靠剩余节点维持服务 | 0（实时冗余） |
| L2       | 数据中心级损毁    | 启动异地冷备份（跨区域恢复）           | < 24 小时     |
| L3       | 数据库故障        | 切换至备用数据库                       | < 24 小时     |
...
系统通过严格的数据库访问控制列表 (ACL) 限制日志修改权限。

**访问控制策略：**
- 仅授权的审计服务账号拥有写入权限
- 严禁任何业务账号或管理员账号直接修改日志表数据
- 所有日志查询操作均记录在审计追踪中
...
| DMZ21  | Yi Deng, Shunli Ma, Xinxuan Zhang, et al. (2021). "Promise σ protocol: How to Construct Efficient Threshold ECDSA from Encryptions Based on Class Groups" | 阶数未知群假设 (Groups of Unknown Order) |
| DKLS23 | Jack Doerner, Yashvanth Kondi, et al. (2023). "Threshold ECDSA in Three Rounds"                                                                           | 随机预言机模型（ROM）                    |
